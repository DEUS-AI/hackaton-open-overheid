...
async function publishToIngestion(
  ...
  // Retry a few times to tolerate emulator warm-up
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      await sender.sendMessages({ body: appMessage, contentType: "application/json", subject: "document_uploaded" });
      return; // success
    } catch (err) {
      lastErr = err;
      const msg = err instanceof Error ? err.message : String(err);
      // Only log after the first failure to avoid noise
